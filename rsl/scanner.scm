...
(define scanner-b
  (lambda (code row col)
    (cond
     ...
     ((char=? #\\ (car code))
	  ...
	  (cons (make-token "\\/" 'or row (+ col 2))
		(scanner-b (cdr code) row (+ col 2)))
	  (report scanner "\\" row col)))
     ((char=? #\! (car code))
      (if (char=? #\! (second code))
	  (cons (make-token "!!" 'cross row (+ col 2))
	...
	(cons (make-token "~=" 'noteq row (+ col 2))
	      (scanner-b (cdr (cdr code)) row (+ col 2))))
       ((string=? (list->string (take-while char-alphabetic? (cdr code)))
		  "isin")
	(cons (make-token "~isin" 'notisin row (+ col 5))
	      (scanner-b (drop 5 code) row (+ col 5))))
       (else
	(cons (make-token "~" 'not row (+ col 1))
     ...
     ((char=? #\+ (car code))
	  ...
	  (cons (make-token "++" 'plusplus row (+ col 2))
		(scanner-b (cdr (cdr code)) row (+ col 2))))
       ((char=? #\> (second code))
	(cons (make-token "+>" 'plusarrow row (+ col 2))
	      (scanner-b (cdr code) row (+ col 2))))
       (else
	(cons (make-token "+" 'plus row (+ col 1))
     ...
     ((char=? #\: (car code))
      (if (char=? #\- (second code))
	  (cons (make-token ":-" 'bullet row (+ col 2))
		(scanner-b (cdr (cdr code)) row (+ col 2)))
	  (cons (make-token ":" 'colon row (+ col 1))
		(scanner-b (cdr code) row (+ col 1)))))
     ((char=? #\> (car code))
      (let ((tok&rst (scan-greater (cdr code) row col)))
	(let ((tok (car tok&rst))
	      (rst (second tok&rst)))
	  (cons tok
    ...
    (cond
     ...
     ((char=? #\\ (car code))
      (list (make-token "-\\" 'lambda row (+ col 2))
	    (cdr code)))
     ((string=? "set" (list->string (take-while char-alphabetic? code)))
      (list (make-token "-set" 'set row (+ col 4))
	    (drop 3 code)))
...
(define scan-bar
  (lambda (code row col)
    (cond
     ...
     ((char=? #\= (car code))
      (if (char=? #\| (second code))
	  (list (make-token "|=|" 'bareqbar row (+ col 3))
		(cdr (cdr code)))
	  (report scanner (char->name (second code)) row (+ col 2))))
     (else
      (report scanner (char->name (car code)) row (+ col 1))))))

;; helper to scan greater chars
(define scan-greater
  (lambda (code row col)
    (cond
     ((char=? #\= (car code))
      (list (make-token ">=" 'greatereq row (+ col 2))
	    (cdr code)))
     ((char=? #\< (car code))
      (list (make-token "><" 'product row (+ col 2))
	    (cdr code)))
     ((char=? #\> (car code))
      (list (make-token ">>" 'propsubset row (+ col 2))
	    (cdr code)))
     ((and (char=? #\> (car code))
	   (char=? #\= (second code)))
      (list (make-token ">>=" 'subset row (+ col 3))
	    (cdr (cdr code))))
     (else
      (list (make-token ">" row (+ col 1))
	    code)))))

;; helper to scan less tokens
(define scan-less
  (lambda (code row col)
    (cond
     ((char=? #\= (car code))
      (list (make-token "<=" 'lesseq row (+ col 2))
	    (cdr code)))
     ((and (char=? #\< (car code))
	   (char=? #\= (second code)))
      (list (make-token "<<=" 'subset row (+ col 3))
     ...
     ((and (char=? #\< (car code))
	   (char=? #\- (second code))
	   (char=? #\> (third code)))
      (list (make-token "<->" 'equiv row (+ col 3))
	    (drop 2 code)))
     ((char=? #\< (car code))
      (list (make-token "<<" 'propersubset row (+ col 2))
	    (cdr code)))
     ((char=? #\. (car code))
      (list (make-token "<." 'ltribracket row (+ col 2))
	    (cdr code)))
     (else
      (list (make-token "<" 'less row (+ col 1))
	    code)))))
